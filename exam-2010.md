...
Ein Formfaktor bezieht sich immer auf 2 Flächen (patches), daher auch die beiden Indizes ``j`` und ``k``. Er beschreibt, welcher Anteil des abgestrahlten Lichtes des einen Patches (``j``) auf dem anderen (``k``) auftrifft. Ist der Sichtkontakt zwischen 2 Patches zum Beispiel nicht gegeben, so ist der Formfaktor 0.  
